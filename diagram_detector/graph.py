...
class Graph(object):
    ...
    def __is_collapse(self, node_a, node_b):
        """
        This function return if exist a intersection in two nodes.
        """
        # [xmin, xmax, ymin, ymax]
        if type(node_a) is list:
            coords_a = node_a
        else:
            coords_a = node_a.get_coordinate()

        coords_b = node_b.get_coordinate()
        x = max(coords_a[0], coords_b[0])
        y = max(coords_a[2], coords_b[2])
        w = min(coords_a[1], coords_b[1]) - x
        h = min(coords_a[3], coords_b[3]) - y
    ...
    def __collapse_nodes(self):
        ...
        *check if exist more than one overlaping text nodes if is the case calculate the distance and the less distance
        it will be the true text in te shape node.
        """
        ...
        for i in range(len(shape_nodes)):
            for j in range(len(text_nodes)):
                if self.__is_collapse(shape_nodes[i], text_nodes[j]):
                    #Check if exist more than one. Set the value of the text of the text_node that are inside of it
                    if collapse_list[j] is None:
    ...
    def __calculate_distance(self, node_a, node_b):
        """
        Compute distance between two rectangle
        """
        ...
        if type(node_a) is list:
            ...
            coords_a = node_a.get_coordinate()
            cx1 = int((coords_a[0] + coords_a[1]) / 2)
            cy1 = int((coords_a[2] + coords_a[3]) / 2)

        coords_b = node_b.get_coordinate()
        cx2 = int((coords_b[0] + coords_b[1]) / 2)
        cy2 = int((coords_b[2] + coords_b[3]) / 2)
