...
# path_text_model = PATH_DIAGRAM_GURU_PROJECT + '/diagram_detector/text_model'
path_text_model = PATH_DIAGRAM_GURU_PROJECT + '/text_model'

source_path = os.path.join(path_text_model, "data_model", f"{source}.hdf5")
output_path = os.path.join(path_text_model, "output", source, arch)
...
class TextClassifier(object):
    ...
    def train_new_data(self):
        ...
        callbacks = self.model.get_callbacks_continue(logdir=output_path, checkpoint=target_path, verbose=0)
    ...
    def recognize(self, image_path):
        ...
        for box in coords:
            ...
            predict, prob = self.model.predict(x=self.__image_generator(crop_img), steps=1, ctc_decode=True, verbose=0)
            predict = [self.dtgen.tokenizer.decode(x[0]) for x in predict]
            text = predict[0]
            nodes.append(Node(coordinate=[xmin, xmax, ymin, ymax], text=text))
        return zip(nodes, images), nodes
